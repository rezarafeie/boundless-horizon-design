-- Drop all versions of can_create_free_trial function
DROP FUNCTION IF EXISTS public.can_create_free_trial(text, text, inet, text);
DROP FUNCTION IF EXISTS public.can_create_free_trial(text, text, text);

-- Create the correct version without IP parameter
...
BEGIN
  ...
  WHERE (
    ...
    (user_device_fingerprint IS NOT NULL AND test_users.user_device_fingerprint = user_device_fingerprint)
  )
